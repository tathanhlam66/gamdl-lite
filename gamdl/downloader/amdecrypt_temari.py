...
Performance notes (Termux / ARM):
  ...
  - Retry sleep uses asyncio.sleep instead of time.sleep so the thread-pool
...
def _decrypt_samples_temari(
    ...
    try:
        ...
        # We record (original_index, enc_bytes, tail_bytes, desc_index) so we
        # can reconstruct the output in submission order after batch decryption.
